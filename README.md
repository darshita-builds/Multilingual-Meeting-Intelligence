...
| **Person A** — STT, diarization, segmentation, extraction | ✅ implemented; `baseline` is still the default until `MOM_*_BACKEND` is switched |
| **Shared** — end-to-end integration testing | ✅ verified — full governed pipeline, live server, real backends; see [`docs/integration.md`](docs/integration.md) |

The pipeline runs on the **rule-based baseline** by default — a real, working
extractor, and the control arm for the required baseline comparison. Person
A's models (`WhisperSTT`, `PyannoteDiarizer`, `EmbeddingAgendaSegmenter`,
`TransformerExtractor`) are implemented and pass their own test suite; enable
them with `MOM_STT_BACKEND=whisper` etc. (see `.env.example`) after
`pip install -r requirements-ml.txt`. See
[`docs/ml-evaluation.md`](docs/ml-evaluation.md) for how they were evaluated
and calibrated, and [`evaluation/report.md`](evaluation/report.md) for the
numbers.
...
With the default `MOM_STT_BACKEND=baseline`, audio uploads produce a
clearly-labelled `[NO TRANSCRIPT AVAILABLE]` placeholder rather than invented
content — so **use the `.txt` fixtures for demos unless `MOM_STT_BACKEND=whisper`
is set** (`pip install -r requirements-ml.txt` first; see `docs/ml-evaluation.md`).
...
```powershell
pytest tests -q                        # 256 tests (180 platform + 76 ML)
...
Docker image build and smoke test. The ML tests mock every model call (see
`docs/ml-evaluation.md`), so they need no GPU, no model download and no
`requirements-ml.txt` install.

Try Person A's pipeline directly, outside the platform entirely:

```powershell
python scripts/run_meeting_pipeline.py --audio fixtures/meetings/01_clean_english_standup.txt
python -m backend.app.ml.evaluation.run_evaluation   # writes evaluation/results.json + report.md
```
...
| # | Item | Owner |
|---|---|---|
| 1 | ~~The four ML modules above~~ — done, see `docs/ml-evaluation.md` | **Person A** |
| 2 | Vector store — decided against; the baseline and embedding segmenters both need no vector store (see `docs/architecture.md`) | decided by A, built by B |
| 3 | ~~End-to-end integration test with a real (non-baseline) ML backend enabled~~ — done, see `docs/integration.md` | **Both** |
...
* [ML evaluation](docs/ml-evaluation.md) — WER, decision/action F1, language breakdown, calibration methodology, limitations
* [Integration](docs/integration.md) — the Person A × Person B seam, what was actually run end to end and how, a real `.env`-loading bug found and fixed
