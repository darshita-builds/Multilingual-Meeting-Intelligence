...
def test_person_a_backends_fail_obviously_rather_than_returning_junk():
    """A broken backend must fail loudly, not silently return an empty result --
    an empty result is indistinguishable from a meeting where nothing was
    decided. `WhisperSTT` now has a real implementation (Person A landed it), so
    the way to prove this is to feed it input it cannot possibly handle and
    check it raises `MLServiceError` rather than, say, returning empty text.
    `TransformerExtractor.extract([])` legitimately has nothing to extract, so
    it is asserted separately to return empty rather than raise."""
    ...
    with pytest.raises(MLServiceError, match="does not exist"):
        WhisperSTT().transcribe(Path("definitely-does-not-exist.wav"))

    result = TransformerExtractor().extract([])
